...
function translateTime(value) {
  ...
  return {time: endHour.toString() + ":" + endMinute.toString(), timePure: parseInt(endHour.toString() + endMinute.toString())};
    /*
  1 %4 -> 30
  2 %4 -> 45
  3 %4 -> 00
...
$(document).ready(function() {
  ...
  $('#ex1').slider({
    ...
    value: translateTimeFromValue(),
      ...
      //  console.log(timeObj);
      $("#timeOfDay").html(translateTime(value[0]).time + "-" + translateTime(value[1]).time);
        return 'Current value: ' + translateTime(value[0]).time + "-" + translateTime(value[1]).time;
      }
  	}
  }); // end of slider
...
function init() {
  ...
  camera = new THREE.PerspectiveCamera( 110, $("#scene").outerWidth() / $("#scene").outerHeight(), 1, 10000 );
...
function loadFloor1() {
  ...
  */
  ...
  material = new THREE.MeshLambertMaterial( { color: 0xBFBFBF, wireframe: false, transparent: true, opacity: 0.4} ); // lambertMaterial to catch light
  ...
  material = new THREE.MeshLambertMaterial( { color: 0xBFBFBF, wireframe: false, transparent: true, opacity: 0.4} );
